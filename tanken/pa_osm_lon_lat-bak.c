...
//				if (node->latitude != READOSM_UNDEFINED && node->latitude <= 54.366725045 && node->latitude >= 54.276634955 && node->longitude != READOSM_UNDEFINED && node->longitude <= 10.213483244 && node->longitude >= 10.059016756) 
					...
					if (node->latitude != READOSM_UNDEFINED && node->latitude <= handle_user_data->max_lat && 
					node->latitude >= handle_user_data->min_lat && node->longitude != READOSM_UNDEFINED && 
					node->longitude <= handle_user_data->max_long && node->longitude >= handle_user_data->min_long) 
						{
							printf (" lat=\"%1.7f\"", node->latitude);
							printf (" lon=\"%1.7f\"", node->longitude);
							printf ("\t\t<tag k=\"%s\" v=\"%s\" />\n name = %s\n", tag->key,tag->value,object_name);
							//printf ("\t\t<tag k=\"%s\" v=\"%s\" />\n",)
							...
							if(dist_2_po < shortest_dist)
							{
								...
								if(strcmp(handle_user_data->arg,"atm") == 0)
								{ 	
									tag = node->tags + 1;
									strcpy(object_name,tag->value);
								}
								if(strcmp(handle_user_data->arg,"parking") == 0)
								{
									tag = node->tags+1;
									strcpy(object_name,tag->value);
								}
